...
export default function CartPoleRenderer({ gameState, width = 500, height = 300 }) {
  ...
  const draw = useCallback(() => {
    ...
    ctx.setLineDash([4, 4])
    ctx.beginPath(); ctx.moveTo(width / 2 - CARTPOLE_CONFIG.xThreshold * scale, cartY - 2)
    ctx.lineTo(width / 2 + CARTPOLE_CONFIG.xThreshold * scale, cartY - 2); ctx.stroke()
    ctx.setLineDash([])

    // Cart body
    ctx.fillStyle = '#3B82F6'
    ctx.shadowColor = '#3B82F6'; ctx.shadowBlur = 10
    ctx.beginPath(); ctx.roundRect(cartX - cartW / 2, cartY - cartH, cartW, cartH, 4); ctx.fill()
    ctx.shadowBlur = 0

    // Wheels
    ctx.fillStyle = '#1E3A5F'
    ctx.beginPath(); ctx.arc(cartX - 15, cartY, 6, 0, Math.PI * 2); ctx.fill()
    ctx.beginPath(); ctx.arc(cartX + 15, cartY, 6, 0, Math.PI * 2); ctx.fill()
    ...
    ctx.beginPath(); ctx.moveTo(cartX, cartY - cartH / 2); ctx.lineTo(poleEndX, poleEndY); ctx.stroke()
    ctx.shadowBlur = 0

    // Pivot
    ctx.fillStyle = '#EAB308'
    ctx.beginPath(); ctx.arc(cartX, cartY - cartH / 2, 5, 0, Math.PI * 2); ctx.fill()
    ...
    if (score >= 195) {
      ctx.fillStyle = '#22C55E'; ctx.font = 'bold 12px Outfit'
